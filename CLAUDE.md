...
The frontend calls the backend directly over HTTP at hardcoded `http://localhost:3000` URLs, all funneled through `app/lib/db.ts` — there is no shared client/server code or generated API types between the two halves, so `app/lib/types.ts` (frontend) and `model.rs`/`schema.rs` (backend) must be kept in sync manually when the API shape changes.
...
- `handlers/` — one file per resource (`classroom.rs`, `student.rs`, `table.rs`), each generally exposing `{resource}_list_handler`, `get_{resource}_handler`, `create_{resource}_handler`, `update_{resource}_handler`, `delete_{resource}_handler`. This CRUD-per-file pattern is the template for a new resource, but it's a default, not a requirement — `table.rs` only exposes `get_classroom_tables_handler` (a scoped list under a classroom) since tables/seats are otherwise written as a unit via the seating-chart endpoints below, not through individual per-table/per-seat CRUD. There is no dedicated `seat.rs` handler file.
- `model.rs` — `sqlx::FromRow` structs mapping directly to DB rows (returned as-is in JSON responses). `SeatModel` is `#[allow(dead_code)]` — seat rows are read via the seating-chart join query (`SeatAssignmentRow` in `classroom.rs`), never deserialized as `SeatModel` directly.
- `schema.rs` — separate `*Schema`/`Update*Schema` structs for request bodies (create takes required fields; update takes all-`Option` fields for partial updates, with a `deserialize_some` helper distinguishing "field omitted" from "field explicitly `null`" for nullable columns like `classroom_id`). `SeatingChartSchema`/`SeatingChartTableSchema` are the exception to the per-resource CRUD shape: they model the *entire* seating chart as one nested payload (`{ tables: [{ x_pos, y_pos, seats: [student_uuid | null, ...] }] }`), where a seat's position in the `seats` array *is* its `position` column — there's no separate seat schema.
...
- `PUT` (`update_seating_chart_handler`, `classroom.rs`) is a full replace inside one transaction: delete every `tables` row for the classroom (cascades to `seats`), then re-insert every table (assigning `table_number` from array index) and every seat (assigning `position` from its index within that table's `seats` array, including empty seats as `NULL` `student_id` rows). There is no diffing — moving one table round-trips the whole chart.
- `GET` (`get_seating_chart_handler`) returns only *occupied* seats (`INNER JOIN` + `student_id IS NOT NULL`) as a flat list of `{table_id, position, student_id}` rows, ordered by `table_number, position` — empty seats are omitted, not returned as nulls. The frontend derives table shape separately via `GET .../tables` (`table.rs`) and reconstructs seat occupancy client-side by joining the two.

### Database schema (`migrations/0001_create_tables.up.sql`)

Four tables: `classrooms`, `students`, `tables`, `seats`. Students optionally reference a `classroom_id`; seats belong to tables (`ON DELETE CASCADE`) and optionally reference a student (`ON DELETE SET NULL`, `UNIQUE` — a student can occupy at most one seat); tables belong to classrooms. `tables.x_pos`/`y_pos` are canvas coordinates (integers, one `GRID_STEP` unit apart); `seats.position` is a seat's index within its table's `seats` array, not a coordinate.
...
- `app/lib/db.ts` holds typed fetch wrappers for every backend call (`getStudent(s)`, `create/update/deleteStudent`, `getClassroom(s)`, `create/update/deleteClassroom`, `getClassroomTables`, `getSeatingChartAssignments`, `updateSeatingChart`) — all routes go through it; there are no inline `fetch("http://localhost:3000/...")` calls left in route files. Extend `db.ts` rather than adding an inline fetch.
- `app/lib/schemas.ts` defines zod schemas per resource (`create*Schema`/`edit*Schema`, plus `seatingChartSchema`) used both for frontend form validation and as the `z.infer<...>` types passed to `db.ts`'s mutation functions.
- `app/lib/types.ts` defines frontend-side `Student`/`Classroom`/`Table`/`SeatAssignment` types independently from the Rust models/schemas (see note in Project overview above about keeping these in sync).
...
- The classroom detail page (`app/routes/classrooms/classroom.tsx`) implements an interactive seating chart on **React Flow** (`@xyflow/react`), replacing an earlier `@dnd-kit/react` implementation. Tables and students are both React Flow nodes (`type: "table" | "student"`, see `nodeTypes` in `app/components/seating-chart-canvas.tsx`, rendered by `app/components/nodes/table-node.tsx` / `student-node.tsx`); a seat assignment is modeled as an `Edge` from a table's per-seat source handle (id from `getSeatHandleId`) to a student node's target handle — there is no separate "seat" node. `useState<nodes>`/`useState<edges>` (driven by React Flow's `onNodesChange`/`onEdgesChange`) hold canvas state directly (no reducer); the `loader` builds the initial nodes/edges once via `buildInitialNodesAndEdges` (memoized with an empty dep array — deliberately not reactive to `loaderData` changes after mount), and `Save` calls `buildSeatingChartPayload` to convert nodes/edges back into a `seatingChartSchema` payload and submits it via `useFetcher`.
- All of this pure, framework-free geometry/id logic (constants like `GRID_STEP`/`CELL`, node/edge shape types, `buildInitialNodesAndEdges`, `buildSeatingChartPayload`, `findNearestSeat`, seat-id helpers) lives in `app/lib/seating-chart-utils.ts`, tested directly in `app/lib/seating-chart-utils.test.ts` — this is the template for any other local, non-persisted-shape UI logic: keep it out of components and out of `db.ts`/`types.ts` (which model actual API-backed shapes).
- Drag/drop uses a threshold-based proximity model rather than exact drop-target hit-testing: dragging a student node within `CONNECT_THRESHOLD` of an unoccupied seat auto-connects it (creating a `"temp"`-classed edge during the drag, finalized in `onNodeDragStop`); dragging an already-seated student past `DISCONNECT_THRESHOLD` (a wider hysteresis band, so small jitters near the boundary don't flicker) hides/removes its edge. Dragging a table also repositions any students connected to it, keeping them pinned to their seat via `getConnectedStudentPosition`.
...
- Non-route pure-logic modules (e.g. `seating-chart-utils.ts`'s node/edge/geometry helpers) are tested directly as plain functions — no fetch mocking or route-args scaffolding needed, just call them.
